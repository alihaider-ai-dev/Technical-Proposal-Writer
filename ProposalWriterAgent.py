...
tools = [getCompanyPortfolio, getRequestForProposalDocument]
scope_of_work = {
...
def CreateAgentExecuter(prompt_content):
    ...
    prompt.messages[
        0].prompt.template = prompt_content  #"You are Expert Proposal Writer for Arweqah Company who has the Information about the our Company and the Request For Proposal i-e the Project Proposal and Scope to which you have to provide the Proposal Parts according to the Given user Query."
...
def InvokeAgent():
    summary = generate_answer(
        ...
        "Your Task is to summarize and Write Objective of the below Request for Proposal (Donot add any prefiex or post just retrun the summary)",
        "claude")
    initiator_prompt = f"""You are Expert Technical Proposal Table of Content or Structure Writer for Arweqah Company (working in social awarness and leadership programs).
    use the Tools getCompanyPortfolio, getRequestForProposalDocument to get the Company Information and the Request For Proposal Inforamtion when required.
        ...
        for i in range(3):
            try:
                result_eng = generate_answer(
                    "", f"""Request for Proposal Information:\n
                    \t\t{read_text_from_pkl("rfpInfo.pkl")} 
                    \n\n Company Information: 
                    ...
                    - Use the Exact Project Timeline mentioned in the Request for Proposal Information and make the whole Content stick to it. Project timeline must not exceed n months as mentioned in the RFP.
